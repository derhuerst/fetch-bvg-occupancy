...
const parseDataset = (dataset, descriptor) => {
	...
	const parseCollection = (items, DM1 = [], root = true, schema = items[0].S) => {
		let currentC = []
		for (const item of items) {
			if (!item.C) continue
			item.C = [...currentC.slice(0, currentC.length - item.C.length), ...item.C]
			currentC = item.C
		}
		...
		.map((item, itemIdx) => {
			...
			if (item.X) {
				_.children = parseCollection(item.X, DM1, false, subSchema)
			}
	...
	const DM1 = parseCollection(dataset.SH[0].DM1)
